...
from sort import *
import json
from pyngrok import ngrok  # Tambahkan ini untuk ngrok
...
def process_video():
    ...
    def non_max_suppression_fast(boxes, overlapThresh):
        ...
        while len(idxs) > 0:
            ...
            yy2 = np.minimum(y2[i], yy2[idxs[:last]])
        ...
        while cap.isOpened():
            ...
            for obj in tracked_objects:
                if not np.isnan(obj).any():
                    ...
                    cv2.putText(frame, f'ID {track_id}', (x1, y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                    cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 2)

            # Simpan frame ke video output
            out.write(frame)

        status_updates.append({"step": "Video processing completed.", "completed": True})
    ...
    status_updates.append({
        "step": f"Video processing completed in {total_process_time:.2f} seconds.",
        "completed": True,
        "total_count": total_count
    })

    return jsonify({'output_video': output_video_name, 'total_count': total_count})

if __name__ == "__main__":
    # Menjalankan ngrok dan server Flask
